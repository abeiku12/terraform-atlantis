...
resource "aws_security_group" "atlantis_sg" {
  name        = "atlantis-sg"
  description = "Allow HTTPS and Atlantis Webhook"
  vpc_id      = "your-vpc-id"
...
resource "aws_ecs_task_definition" "atlantis_task" {
  ...
  execution_role_arn = aws_iam_role.atlantis.arn
  task_role_arn      = aws_iam_role.atlantis.arn
...
resource "aws_cloudwatch_metric_alarm" "cpu_high" {
  ...
  dimensions = {
    ClusterName = aws_ecs_cluster.atlantis_cluster.name
    ServiceName = aws_ecs_service.atlantis_service.name
  }
  actions_enabled = true
  alarm_actions  = ["arn:aws:sns:us-west-2:123456789012:ecs-alerts"]
}
